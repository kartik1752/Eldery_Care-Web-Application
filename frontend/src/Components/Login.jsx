  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(
        `${import.meta.env.VITE_BACKEND_URL}/api/login/check`,
        {
          method: "POST",
          headers: {
      ...
      if (response.ok) {
        // Convert returned role to lowercase ("elder", "family", "admin")
        const normalizedRole = data.role ? data.role.toLowerCase() : "";

        // Save JWT TOKEN & User Info using keys expected by ProtectedRoute & Dashboards
    ...
    <div className="flex items-center justify-center min-h-screen bg-[#F9EDE1] px-4">
      <div className="flex flex-col md:flex-row bg-white rounded-2xl shadow-lg overflow-hidden max-w-4xl w-full">
        ...
        <div className="w-full md:w-1/2 p-6 md:p-8 flex flex-col justify-center">
          <h2 className="text-2xl md:text-3xl font-bold mb-6 text-[#5A4230] text-center">
            Welcome Back
          </h2>
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div className="mb-4">
              <label
                htmlFor="email"
                className="block text-sm font-medium text-[#5A4230]"
              >
                Email
              </label>
              <input
          ...
          <p className="mt-4 text-sm text-gray-600 text-center">
            ...
            <Link to="/signup" className="text-[#C48F65] hover:underline">
              Sign Up
            </Link>
          </p>
        </div>
