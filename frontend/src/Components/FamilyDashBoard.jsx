...
export const FamilyDashBoard = () => {
  ...
  useEffect(() => {
    fetch(`${import.meta.env.VITE_BACKEND_URL}/api/elder/getElders`, {
      ...
      .then((res) => res.json())
      .then((data) => setElders(Array.isArray(data) ? data : []))
  ...
  return (
    <div className="min-h-screen bg-[#F9EDE1] flex flex-col items-center p-4">
      <div className="w-full max-w-3xl bg-white shadow-xl rounded-lg p-6">
        ...
        <div className="mt-6">
          <h3 className="text-lg font-semibold text-[#5A4230]">Assign a Task to Your Elder</h3>
        ...
        {selectedElder && (
          <div className="mt-6">
            <h3 className="text-lg font-semibold text-[#5A4230]">Tasks for {selectedElder}</h3>
  ...
  tasks.map((t) => (
    <motion.div
      ...
      className="flex items-center bg-gray-100 p-4 shadow-md rounded-lg w-full justify-between"
    >
      <div className="flex items-center min-w-0">
        <FaTasks className="text-blue-500 mr-3 shrink-0" size={20} />
        <div className="min-w-0">
          <h4 className="text-md font-bold text-[#5A4230] truncate">{t.task}</h4>
        </div>
      </div>
      <span
